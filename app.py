...
import os, json, random,sqlite3,traceback
...
def get_standings(tname):
    try:
        ...
        # ----------------------------------------------------
        # Fetch all rounds (can be empty)
        # ----------------------------------------------------
        rounds = Round.query.filter_by(tournament_id=tournament.id).order_by(Round.round_number.asc()).all()

        # ----------------------------------------------------
        # result_map[(player_id, opponent_id)] = 1/0.5/0
        # ----------------------------------------------------
        result_map = {}
        ...
        for rnd in rounds:
            ...
            for m in pairings:
                ...
                if res == "white":
                    ...
                    # bye_white / bye_black etc -> ignore for TB calculations
                    pass

        # ----------------------------------------------------
        # Helper: safe parse opponents
        # ----------------------------------------------------
        ...
        # TB2..TB6 computed here
        # ----------------------------------------------------
        standings = []

        # Pre-calc: wins as black using rounds data (not from p.wins etc)
        wins_as_black_map = {p.id: 0 for p in participants}

        for rnd in rounds:
            pairings = json.loads(rnd.pairings) if rnd.pairings else []
            for m in pairings:
                if m.get("result") == "black":
                    black_id = m.get("black_id")
                    if black_id in wins_as_black_map:
                        wins_as_black_map[black_id] += 1

        for p in participants:
            opponents_list = safe_json_list(getattr(p, "opponents", None))

            # Buchholz: sum of opponents' scores
            opp_scores = []
            for oid in opponents_list:
                opp = participant_by_id.get(oid)
                if opp:
                    opp_scores.append(float(getattr(opp, "score", 0) or 0))

            buchholz = float(sum(opp_scores))
            buchholz_cut1 = float(sum(sorted(opp_scores)[1:])) if len(opp_scores) > 1 else buchholz
            ...
            for oid in opponents_list:
                ...
                if not opp:
                    continue
                r = result_map.get((p.id, oid), None)
                if r is None:
                    continue
                sb += float(getattr(opp, "score", 0) or 0) * float(r)

            # Most wins (count actual wins vs opponents)
            wins = 0
            for oid in opponents_list:
                if result_map.get((p.id, oid), None) == 1.0:
                    wins += 1

            white_count = int(getattr(p, "white_count", 0) or 0)
            black_count = int(getattr(p, "black_count", 0) or 0)
            games_played = white_count + black_count
            bye_count = int(getattr(p, "bye_count", 0) or 0)

            standings.append({
                "id": p.id,
                "name": p.name,
                "elo": int(getattr(p, "elo", 0) or 0),
                "score": float(getattr(p, "score", 0) or 0),

                # TBs
                "tb1_direct": None,                 # TB1 computed later
                "buchholz": buchholz,               # TB2
                "buchholz_cut1": buchholz_cut1,     # TB3
                "sb": sb,                           # TB4
                "wins": int(wins),                  # TB5
                "wins_black": int(wins_as_black_map.get(p.id, 0)),  # TB6
        ...
        # ----------------------------------------------------
        # TB1 MINI LEAGUE (Direct encounter)
        # Apply only within SCORE-tied groups
        # ----------------------------------------------------

        def compute_mini_league(players_in_group):
            """
            Apply TB1 only if ALL players in this tied score group have played each other.
            Otherwise TB1 remains None for everyone.
            """
            ids = [x["id"] for x in players_in_group]
            ...
            # -------------------------------------------
            # Check if this tie group is a complete mini-league
            # i.e., every pair has a result recorded
            # -------------------------------------------
            played_pairs = set()  # {(min_id, max_id), ...}

            for pid in ids:
                for oid in ids:
                    if pid >= oid:
                        continue
                    # if either direction exists in result_map, game was played
                    if (pid, oid) in result_map or (oid, pid) in result_map:
                        played_pairs.add((pid, oid))

            required_pairs = n * (n - 1) // 2

            # Not a complete mini-league -> TB1 not applicable
            if len(played_pairs) != required_pairs:
                for row in players_in_group:
                    row["tb1_direct"] = None
                return players_in_group  # keep base order (TB2..TB6 already sorted)

            # -------------------------------------------
            # Compute mini-league scores (only vs players in this group)
            # -------------------------------------------
            mini_scores = {pid: 0.0 for pid in ids}

            for pid in ids:
                for oid in ids:
                    if pid == oid:
                        continue
                    r = result_map.get((pid, oid), None)
                    if r is None:
                        continue
                    mini_scores[pid] += float(r)

            # show tb1 only if played direct encounter at least once
            for row in players_in_group:
                row["tb1_direct"] = mini_scores[row["id"]]

            players_in_group.sort(key=lambda x: (
                -x["tb1_direct"],       # TB1
                -x["buchholz"],     # TB2
                -x["buchholz_cut1"],        # TB3
                -x["sb"],       # TB4
        ...
        final_standings = []
        for score, group_iter in groupby(standings, key=lambda x: x["score"]):
            group_list = list(group_iter)
            if len(group_list) >= 2:
                group_list = compute_mini_league(group_list)
        ...
        return jsonify({
            ...
            "total_rounds": int(getattr(tournament, "rounds", 0) or 0),
...
@app.route('/api/tournament/<int:tournament_id>', methods=['DELETE'])
def delete_tournament(tournament_id):
    try:
        ...
        # Get tournament name before deleting
        c.execute("SELECT name FROM tournament WHERE id = ?", (tournament_id,))
        result = c.fetchone()
        ...
        # Then delete rounds
        c.execute("DELETE FROM round WHERE tournament_id = ?", (tournament_id,))
        
        # Then delete participants
        c.execute("DELETE FROM participant WHERE tournament_id = ?", (tournament_id,))
        
        # Finally delete the tournament
        c.execute("DELETE FROM tournament WHERE id = ?", (tournament_id,))
        ...
        print(f"Error deleting tournament: {str(e)}")  # Debug log
